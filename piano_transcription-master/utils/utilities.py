...
class TargetProcessor(object):
    ...
    def process(self, start_time, midi_events_time, midi_events, 
        ...
        pedal_buffer_dict = {}     # Used to store onset of pedal to be paired with offset of pedal
        ...
        for i in range(ex_bgn_idx, fin_idx):
            ...
            # Pedal
            elif attribute_list[0] == 'control_change' and attribute_list[2][-2:] in ['64', '66', '67']:
                """control=64 corresponds to pedal MIDI event. E.g. 
                attribute_list: ['control_change', 'channel=0', 'control=64', 'value=45', 'time=43']"""

                ped_type = int(attribute_list[2][-2:])
                ped_velocity = int(attribute_list[3].split('=')[1])

                # Onset
                if ped_velocity > 0: 
                    if ped_type in pedal_buffer_dict.keys():
                        pedal_events.append({
                            'ped_type': ped_type,
                            'onset_time': pedal_buffer_dict[ped_type]['onset_time'], 
                            'offset_time': midi_events_time[i],
                            'velocity': pedal_buffer_dict[ped_type]['velocity']})
                        del pedal_buffer_dict[ped_type]
                    
                    pedal_buffer_dict[ped_type] = {
                        'onset_time' : midi_events_time[i],
                        'velocity': ped_velocity
                    }       
        ...
        for ped_type in pedal_buffer_dict.keys():
            pedal_events.append({
                'ped_type': ped_type,
                'onset_time': pedal_buffer_dict[ped_type]['onset_time'], 
                'offset_time': start_time + self.segment_seconds,
        ...
        """Get regression targets will be done after pedal roll concatenated"""
        ...
        for pedal_event in pedal_events:
            """pedal_event: e.g., {'ped_type': 64, 'onset_time': 722.0719, 'offset_time': 722.47815, 'velocity': 103}"""

            """follow the index after 88 piano notes, add to the end of rolls"""
            if pedal_event['ped_type'] == 64: ped_index = 88
            if pedal_event['ped_type'] == 66: ped_index = 89
            if pedal_event['ped_type'] == 67: ped_index = 90
            ...
            if fin_frame >= 0:
                frame_roll[max(bgn_frame, 0) : fin_frame + 1, ped_index] = 1

                offset_roll[fin_frame, ped_index] = 1
                velocity_roll[max(bgn_frame, 0) : fin_frame + 1, ped_index] = pedal_event['velocity']

                # Vector from the center of a frame to ground truth offset
                reg_offset_roll[fin_frame, ped_index] = \
                ...
                if bgn_frame >= 0:
                    ...
                    # Vector from the center of a frame to ground truth onset
                    reg_onset_roll[bgn_frame, ped_index] = \
                        (pedal_event['onset_time'] - start_time) - (bgn_frame / self.frames_per_second)
            
                # Mask out segment notes
                else:
                    mask_roll[: fin_frame + 1, ped_index] = 0
...
def write_events_to_midi(start_time, note_events, pedal_events, midi_path):
    ...
    if pedal_events:
        for pedal_event in pedal_events:
            message_roll.append({'time': pedal_event['onset_time'], 'control_change': pedal_event['ped_type'], 'value': pedal_event['velocity']})
            message_roll.append({'time': pedal_event['offset_time'], 'control_change': pedal_event['ped_type'], 'value': 0})
...
class RegressionPostProcessor(object):
    ...
    def output_dict_to_midi_events(self, output_dict):
        ...
        (est_on_off_note_vels, est_on_off_pedal_vels) = \
        ...
        est_on_off_pedal_vels: (pedal_events_num, 4), the four columns are: [onset_time, offset_time, pedal_type, velociy]"""

        # Reformat notes to MIDI events
        est_note_events = self.detected_notes_to_events(est_on_off_note_vels)

        if est_on_off_pedal_vels is None:
            est_pedal_events = None
        else:
            est_pedal_events = self.detected_pedals_to_events(est_on_off_pedal_vels)
    ...
    def output_dict_to_note_pedal_arrays(self, output_dict):
        ...
        est_on_off_note_vels, est_on_off_pedal_vels = self.output_dict_to_detected_notes_pedals(output_dict)
  

        return est_on_off_note_vels, est_on_off_pedal_vels
    ...
    def output_dict_to_detected_notes_pedals(self, output_dict):
        ...
        for index in range(classes_num):
            ...
            est_tuples_per_note = note_detection_with_onset_offset_regress(
                frame_output=output_dict['frame_output'][:, index], 
                onset_output=output_dict['onset_output'][:, index], 
                onset_shift_output=output_dict['onset_shift_output'][:, index], 
                offset_output=output_dict['offset_output'][:, index], 
                offset_shift_output=output_dict['offset_shift_output'][:, index], 
                velocity_output=output_dict['velocity_output'][:, index], 
                frame_threshold=self.frame_threshold)
            
            
            if index < 88: # piano note
                est_tuples_notes += est_tuples_per_note
                est_midi_notes += [index + self.begin_note] * len(est_tuples_per_note)

            else: # pedals
                est_tuples_pedals += est_tuples_per_note
                if index == 88: ped_type = 64
                if index == 89: ped_type = 66
                if index == 90: ped_type = 67
                est_pedal_types += [ped_type] * len(est_tuples_per_note)

        est_tuples_notes = np.array(est_tuples_notes)   # (notes, 5)
        ...
        onset_times = (est_tuples_notes[:, 0] + est_tuples_notes[:, 2]) / self.frames_per_second
        offset_times = (est_tuples_notes[:, 1] + est_tuples_notes[:, 3]) / self.frames_per_second
        velocities = est_tuples_notes[:, 4]
        ...
        """process pedals"""
        est_tuples_pedals = np.array(est_tuples_pedals) 
        est_pedal_types = np.array(est_pedal_types)

        onset_times_pedal = (est_tuples_pedals[:, 0] + est_tuples_pedals[:, 2]) / self.frames_per_second
        offset_times_pedal = (est_tuples_pedals[:, 1] + est_tuples_pedals[:, 3]) / self.frames_per_second
        velocities_pedal = est_tuples_pedals[:, 4]
        est_on_off_pedal_vels = np.stack((onset_times_pedal, offset_times_pedal, est_pedal_types, velocities_pedal), axis=-1)
        est_on_off_pedal_vels = est_on_off_pedal_vels.astype(np.float32)
    ...
    def detected_pedals_to_events(self, est_on_off_pedal_vels):
        ...
        """
        pedal_events = []
        for i in range(len(est_on_off_pedal_vels)):
            pedal_events.append({
                'onset_time': est_on_off_pedal_vels[i, 0], 
                'offset_time': est_on_off_pedal_vels[i, 1],
                'ped_type': int(est_on_off_pedal_vels[i][2]), 
                'velocity': int(est_on_off_pedal_vels[i][3] * self.velocity_scale)})
